...
class EMBXPropertyStore
{
    ...
    public static function GetPropertyDescription()
    {
        return array(
            "PROPERTY_TYPE" => "S",
            "USER_TYPE" => 'EMBXStore',
            "DESCRIPTION" => GetMessage('EMBXPA_STORE_DESCR'),
    ...
    static function GetAdminListViewHTML($arProperty, $value, $strHTMLControlName){
    	if($arStore = self::getElement($value)){
    		return '[' . $arStore["ID"] . '] ' . $arStore["TITLE"];
    	}else{
    		return $value["VALUE"];
    	}
    }
    ...
    {
        $settings = EMBXPropertyStore::PrepareSettings($arProperty);
    ...
    static function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
    {
        ...
        $bWasSelect = false;
        $options = EMBXPropertyStore::GetOptionsHtml($arProperty, (is_array($value["VALUE"])) ?  $value["VALUE"] : array($value["VALUE"]), $bWasSelect);
    ...
    static function GetPropertyFieldHtmlMulty($arProperty, $value, $strHTMLControlName)
    {
        ...
        {
            $bWasSelect = false;
            $options = EMBXPropertyStore::GetOptionsHtml($arProperty, $values, $bWasSelect);
        ...
        else
        {
            ...
            foreach($values as $property_value_id=>$value)
            {
                ...
                $bWasSelect = false;
                $options = EMBXPropertyStore::GetOptionsHtml($arProperty, array($value), $bWasSelect);
    ...
    static function GetOptionsHtml($arProperty, $values, &$bWasSelect)
    {
        ...
        foreach(EMBXPropertyStore::GetElements() as $arItem)
        {
        	...
        	$options .= '>['. $arItem["ID"] . '] ' . $arItem["TITLE"].'</option>';
        }        
	...
	static function GetElements()
	{
		...
		if(empty($cache))
		{
		    $res = CCatalogStore::GetList(array("ID" => "ASC", "NAME" => "ASC"), array(), false, false);
            $cache = array();
