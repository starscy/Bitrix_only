...
class EMBXUserTypeDelivery extends \Bitrix\Main\UserField\Types\EnumType {
	
	public const
		USER_TYPE_ID = 'EMBXDelivery',
		DISPLAYS = [
				self::DISPLAY_LIST,
	...
	public static function getDescription(): array {
		return [
				'DESCRIPTION' => Loc::getMessage('EMBXPA_USER_TYPE_DELIVERY_DESCRIPTION'),
	...
	public static function getList(array $userField, $id = null) {
		...
		$res = \Bitrix\Sale\Delivery\Services\Manager::getList([
		...
		$result = [];
		foreach ($res as $item){
			$code = !empty($item["CODE"]) ? $item["CODE"] : $item["ID"];
			$result[] = [
					"ID" => $item["ID"],
