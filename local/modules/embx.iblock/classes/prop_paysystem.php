...
class EMBXPropertyPS
{
    ...
    public static function GetPropertyDescription()
    {
        return array(
            "PROPERTY_TYPE" => "S",
            "USER_TYPE" => 'EMBXPaySystem',
            "DESCRIPTION" => GetMessage('EMBXPA_PS_DESCR'),
    ...
    static function GetAdminListViewHTML($arProperty, $value, $strHTMLControlName){
    	if($arStore = self::getElement($value)){
    		return '[' . $arStore["ID"] . '] ' . $arStore["NAME"];
    	}else{
    		return $value["VALUE"];
    	}
    }
    ...
    {
        $settings = EMBXPropertyPS::PrepareSettings($arProperty);
    ...
    static function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
    {
        ...
        $bWasSelect = false;
        $options = EMBXPropertyPS::GetOptionsHtml($arProperty, (is_array($value["VALUE"])) ?  $value["VALUE"] : array($value["VALUE"]), $bWasSelect);
    ...
    static function GetPropertyFieldHtmlMulty($arProperty, $value, $strHTMLControlName)
    {
        ...
        {
            $bWasSelect = false;
            $options = EMBXPropertyPS::GetOptionsHtml($arProperty, $values, $bWasSelect);
        ...
        else
        {
            ...
            foreach($values as $property_value_id=>$value)
            {
                ...
                $bWasSelect = false;
                $options = EMBXPropertyPS::GetOptionsHtml($arProperty, array($value), $bWasSelect);
    ...
    static function GetOptionsHtml($arProperty, $values, &$bWasSelect)
    {
        ...
        foreach(EMBXPropertyPS::GetElements() as $arItem)
        {
	...
	static function GetElements()
	{
		...
		if(empty($cache))
		{		    
		    $res = CSalePaySystem::GetList(array("ID" => "ASC", "NAME" => "ASC"), array(), false, false);
            $cache = array();
