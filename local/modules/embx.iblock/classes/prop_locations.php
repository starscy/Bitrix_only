...
class EMBXPropertyLocations
{
    ...
    public static function GetPropertyDescription()
    {
        return array(
            "PROPERTY_TYPE" => "S",
            "USER_TYPE" => 'EMBXLocations',
            "DESCRIPTION" => GetMessage('EMBXPA_LOCATION_DESCR'),
    ...
    static function getElement($value){
    	if(!empty($value["VALUE"])){    	
    		...
    		$rsLocationsList = CSaleLocation::GetList(
    				...
    				array("LID" => LANGUAGE_ID, "ID" => intval($value["VALUE"]))
    		);
    ...
    static function GetAdminListViewHTML($arProperty, $value, $strHTMLControlName){
    	if($arCity = self::getElement($value)){
   			return '[' . $arCity["ID"] . '] ' . $arCity["CITY_NAME_LANG"] . 
    				((!empty($arCity["REGION_NAME"])) ? ' ' . $arCity["REGION_NAME"] : '') . 
    				((!empty($arCity["COUNTRY_NAME_LANG"])) ? ' ' . $arCity["COUNTRY_NAME_LANG"] : '');
    	}else{
    		return $value["VALUE"];
    	}
    }
   ...
   static function PrepareSettings($arProperty)
    {
        ...
        if(is_array($arProperty["USER_TYPE_SETTINGS"]) && $arProperty["USER_TYPE_SETTINGS"]["regions"] === "Y")
        	$regions= "Y";
       	else
       		$regions= "N";
    ...
    {
        $settings = EMBXPropertyLocations::PrepareSettings($arProperty);
	...
	<tr valign="top">
	<td>'.GetMessage("EMBXPA_SETTING_REGIONS").':</td>
	<td><input type="checkbox" name="'.$strHTMLControlName["NAME"].'[regions]" value="Y" '.($settings["regions"]=="Y"? 'checked': '').'></td>
	</tr>
	';
    }
    ...
    static function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
    {
        ...
        $bWasSelect = false;
        $options = EMBXPropertyLocations::GetOptionsHtml($arProperty, (is_array($value["VALUE"])) ?  $value["VALUE"] : array($value["VALUE"]), $bWasSelect);
    ...
    static function GetPropertyFieldHtmlMulty($arProperty, $value, $strHTMLControlName)
    {
        ...
        {
            $bWasSelect = false;
            $options = EMBXPropertyLocations::GetOptionsHtml($arProperty, $values, $bWasSelect);
        ...
        else
        {
            ...
            foreach($values as $property_value_id=>$value)
            {
                ...
                $bWasSelect = false;
                $options = EMBXPropertyLocations::GetOptionsHtml($arProperty, array($value), $bWasSelect);
    ...
    static function GetOptionsHtml($arProperty, $values, &$bWasSelect)
    {
        ...
        foreach(EMBXPropertyLocations::GetElements($settings) as $arItem)
        {
            ...
            $options .= '>'. (isset($arItem["CITY"]) ? "&nbsp; &nbsp; &nbsp;" : "") .  $arItem["NAME"].'</option>';
        }

        return  $options;
    }

    static function GetElements($settings)
	{
		static $cache = array();
        CModule::IncludeModule("sale");
		...
		if(empty($cache))
		{
            ...
            $rsLocationsList = CSaleLocation::GetList(
                array("SORT" => "ASC", "COUNTRY_NAME_LANG" => "ASC", "REGION_NAME_LANG" => "ASC", "CITY_NAME_LANG" => "ASC"),
                array("LID" => LANGUAGE_ID),
                false, false,
                array("ID", "CITY_ID", "CITY_NAME", "COUNTRY_ID", "COUNTRY_NAME", "REGION_NAME", "REGION_ID")
            );

            while ($arCity = $rsLocationsList->GetNext()) {
            	if(isset($settings["regions"]) && $settings["regions"] == "Y"){
            		if ($arCity["REGION_ID"] != '' && $arCity["CITY_ID"] == '') {
	            		$cache[] = array("ID" => $arCity["REGION_ID"], "NAME" => $arCity["REGION_NAME"]);
	            	}
            	}
                if ($arCity["CITY_ID"] != '') {
                    $cache[] = array("ID" => $arCity["ID"], "NAME" => $arCity["CITY_NAME"], "CITY" => true);
                }
            }
		}
