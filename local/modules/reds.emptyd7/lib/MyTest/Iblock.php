...
class Iblock
{
    ...
    {
        if ($arFields['IBLOCK_ID'] === getIDBlockByCode('LOG')) return ;

        $resSection = new \CIBlockSection();
        $resElement = new \CIBlockElement();

        $el = \CIBlockElement::GetList([],['ID' => $arFields["ID"]], false, false, ['ID', 'NAME' ,'CODE', 'IBLOCK_NAME', 'IBLOCK_CODE', 'IBLOCK_SECTION_ID' ])->GetNext();
        $section = \CIBlockSection::GetList([], ['ID' => $el['IBLOCK_SECTION_ID']], false, false, ['ID', 'IBLOCK_NAME'])->GetNext();

        $stringPath = '';
        $list = \CIBlockSection::GetNavChain(false, $section['ID'], array(), true);

        foreach ($list as $arSectionPath){
            $stringPath .=  '-> ' . $arSectionPath['NAME'];
        }

        $arLoadSectionArray = [
            "IBLOCK_ID" => getIDBlockByCode('LOG'),
            "NAME" => $el["IBLOCK_NAME"],
            "CODE" => $el["IBLOCK_CODE"],
            "ACTIVE" => 'Y',
        ];

        $resSection->Add($arLoadSectionArray);

        $sectionRes = \CIBlockSection::GetList([],["CODE" => $el["IBLOCK_CODE"]])->GetNext();

        $arLoadElementArray = [
            "ACTIVE" => 'Y',
            "IBLOCK_ID" => getIDBlockByCode('LOG'),
            "NAME" => $el["ID"],
            "ACTIVE_FROM" => $el["TIMESTAMP_X"],
            "PREVIEW_TEXT" => $el["IBLOCK_NAME"] . $stringPath . ' ->' . $el['NAME'] . '.',
            "IBLOCK_SECTION_ID" =>$sectionRes['ID'],
        ];

        $resElement->Add($arLoadElementArray);
    }
